...
class GameAgent:
    ...
    def make_move(self, game: Game) -> Tuple[int, int] | int:
        ...
        elif game.get_type() in [
            Type.CONNECT_4_BASIC,
            Type.CONNECT_4_EXTENDED,
            Type.CONNECT_4_MULTIPLAYER,
            Type.CONNECT_4_HIDDEN_MULTIPLAYER,
        ]:
    ...
    def cf_move(self, game: Game):
        ...
        # Win if possible
        move = GameAgent._find_if_winning_move_c4_possible(
            board, self._token, how_many_to_win
        )
        if move is not None:
            return move
        ...
        for token_opp in token_opps:
            move = GameAgent._find_if_winning_move_c4_possible(
                board, token_opp, how_many_to_win
            )
            if move is not None:
                return move
        ...
        columns_from_middle = list(range(cols // 2, cols)) + list(
            range(cols // 2 - 1, -1, -1)
        )
    ...
    def ttt_move(self, game: Game):
        ...
        token_opp = (
            game.player2_token()
            if game.player1_token() == self._token
            else game.player1_token()
        )
